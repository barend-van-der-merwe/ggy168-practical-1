...
- Select folder where graded copies should be saved
- Click on the LOAD button
- Confirm  student details are correct, if not, enter correct student number and press update button
- Select the check boxes where the answer was correct (refer to memo)
...
if image is not None:
    ...
    aruco_dictionary = cv.aruco.Dictionary_get(cv.aruco.DICT_4X4_50)
    aruco_params = cv.aruco.DetectorParameters_create()
    corners, ids, _ = cv.aruco.detectMarkers(img_gray, aruco_dictionary, parameters=aruco_params)
    ...
    if st.button("Grade"):
        ...
        final_img = cv.putText(img=img, text=f'{q1_grade}', org=(588, 548), fontFace=cv.FONT_HERSHEY_SIMPLEX,
                               fontScale=1, color=(0, 0, 255), thickness=2)
        final_img = cv.putText(img=final_img, text=f'{q2_grade}', org=(588, 700),
                               fontFace=cv.FONT_HERSHEY_SIMPLEX,
                               fontScale=1, color=(0, 0, 255), thickness=2)
        final_img = cv.putText(img=final_img, text=f'{final_grade}', org=(400, 260), fontFace=cv.FONT_HERSHEY_SIMPLEX,
                               fontScale=1, color=(0, 0, 255), thickness=2)
        st.image(final_img)
